...
def parse_args():
    parser = argparse.ArgumentParser(description='Copy .txt files from a Docker container')
    parser.add_argument('--container_name', help='Docker container name', required=True)
    parser.add_argument('--source_path', help='Path inside the container to search for .txt files', required=True)
...
if __name__ == "__main__":
    ...
    # Find .txt files in the source path inside the container and get a list of them
    find_command = f"sudo docker exec {container_name} find {source_path} -type f -name '*.txt'"
